...
Prerequisites: set `AZURE_OPENAI_ENDPOINT` and `AZURE_OPENAI_CHAT_DEPLOYMENT_NAME`,
plus either `AZURE_OPENAI_API_KEY` or authenticate with Azure CLI before starting the Functions host.
"""
...
# Helper to get the correct credential for local dev vs deployed
def _get_credential():
...
chat_client = AzureOpenAIChatClient(credential=_get_credential())
